...
/** Spines sit flush at this local z (relative to the book's slot). */
export const spineFrontZ = 0.03;

/**
 * Books have different cover widths, so each one gets its own shelved depth
 * that keeps its spine flush with the shelf front. The shared rotation lane
 * (computed from the widest book) stays clear for all of them.
 */
export function layoutForBook(base: MotionLayout, width: number): MotionLayout {
  return { ...base, shelvedZ: spineFrontZ - width * 0.5 };
}

/** Cover board width in scene units for a book of the given height. */
export function coverWidthFor(height: number) {
  return Math.min(1.36, Math.max(1.12, height * 0.63));
}
