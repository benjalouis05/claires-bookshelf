...
function drawCoverTypography(
  ...
  ctx.fillText(book.series?.toUpperCase() ?? siteConfig.coverImprint, 82, 74, width - 164);
...
/** Procedural front cover, used when a book has no cover image. */
export function createFrontCover(book: Book) {
...
function authorLine(book: Book) {
  return book.author.replace(/^Edited by /, "");
}

/**
 * Draws a spine into `ctx` at the given pixel rectangle. Drawing happens in a
 * logical space that has the book's real spine proportions (thickness ×
 * height), then gets stretched into the cell; the stretch is undone when the
 * texture is mapped back onto the 3D spine, so type never looks squashed.
 */
export function drawSpine(
  ...
  width: number,
  height: number,
  detail: "atlas" | "hero" = "atlas",
) {
  const logicalHeight = 2048;
  const logicalWidth = Math.max(110, (logicalHeight * book.thickness) / book.height);
  ctx.save();
  ctx.beginPath();
  ...
  ctx.fillRect(0, 0, logicalWidth, logicalHeight);
  if (detail === "hero") {
    addPaperGrain(ctx, logicalWidth, logicalHeight, `${book.id}-spine`, 500);
  }

  // Head and tail bands in the accent color.
  ctx.fillStyle = book.accent;
  ctx.globalAlpha = 0.85;
  ctx.fillRect(0, 70, logicalWidth, 10);
  ...
  ctx.save();
  ctx.translate(
    hasAuthorRow ? logicalWidth * 0.4 : logicalWidth * 0.5,
    logicalHeight - 250,
  );
...
export function createSpineCover(book: Book) {
  const canvas = makeCanvas(128, 1024);
  const ctx = canvas.getContext("2d");
  if (ctx) drawSpine(ctx, book, 0, 0, canvas.width, canvas.height, "hero");
  return canvas;
}
...
/** Back cover: the reader's own record of the book. */
export function createBackCover(book: Book) {
  const logicalWidth = 1024;
  const logicalHeight = 1536;
  const canvas = makeCanvas(512, 768);
  const ctx = canvas.getContext("2d");
  ...
  ctx.fillText(siteConfig.editionEyebrow, 108, 150);

  ctx.letterSpacing = "0px";
  ctx.font = `500 64px ${serif}`;
  const lines = wrapText(ctx, book.title, 108, 210, 808, 70, 4);
  ctx.font = `italic 500 40px ${serif}`;
  ctx.fillText(authorLine(book), 108, 230 + lines * 70, 808);

  let y = 360 + lines * 70;
  ctx.save();
  ctx.globalAlpha = 0.9;
  ctx.fillStyle = book.accent;
  ctx.fillRect(108, y, 108, 9);
  ctx.restore();
  y += 60;

  if (book.myRating) {
    ctx.font = `400 88px ${serif}`;
    ctx.fillText("★".repeat(book.myRating), 104, y);
    ctx.globalAlpha = 0.35;
    ctx.fillText(
      "★".repeat(5 - book.myRating),
      104 + ctx.measureText("★".repeat(book.myRating)).width,
      y,
    );
    ctx.globalAlpha = 1;
    y += 140;
  ...
  const facts: Array<[string, string | null]> = [
    ["Read", formatMonthYear(book.dateRead)],
    ["Shelved", formatMonthYear(book.dateAdded)],
    ["Pages", book.pages ? book.pages.toLocaleString("en-US") : null],
    ["Goodreads", book.avgRating ? `${book.avgRating.toFixed(2)} average` : null],
  ];
  facts
